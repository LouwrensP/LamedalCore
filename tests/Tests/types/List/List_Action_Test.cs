...
{
    public partial class xTypes_List // List_Action_Test
    {
        [Fact]
        ...
        public void UniqueAndSort_Test()
        {
            ...
            Assert.Equal(new[] { "Duplicate", "item1", "Item1", "Item2", "Item3" }, _lamed.Types.List.Action.Unique(items1, enCompare_Sort.Ascending).ToArray());
            Assert.Equal(new[] { "Item3","Item2","Item1","Duplicate"}, _lamed.Types.List.Action.Unique(items1, enCompare_Sort.Descending, true).ToArray());
            Assert.Equal(null, _lamed.Types.List.Action.Unique(null, enCompare_Sort.Ascending, true));
